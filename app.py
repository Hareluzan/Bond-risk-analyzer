...
def **init**(self, ytm, duration, rating, net_debt_ebitda, current_ratio, coverage_ratio):
self.ytm = ytm
...
fig.add_annotation(x=0.5, y=0.28, text=f”<b>{score}</b>”, showarrow=False,
...
REQUIRED_PARAMS  = [“Total_Debt”, “Cash”, “EBITDA”, “Current_Assets”,
“Current_Liabilities”, “Operating_Profit”, “Interest_Expense”]
...
def build_export_csv(analyzer, final_score, net_debt_ebitda, current_ratio, coverage_ratio,
bond_name, ytm, duration, rating):
summary = {
“שם האג"ח”:          bond_name,
“תשואה לפדיון (%)”:    ytm,
“מח"מ (שנים)”:         duration,
...
with tab_single:
    ...
    template_data = {"Parameter": REQUIRED_PARAMS,
                     "Value":     [1200, 150, 280, 620, 390, 200, 45]}
    csv_template = pd.DataFrame(template_data).to_csv(index=False).encode('utf-8-sig')

    col_up, col_dl = st.columns([2, 1])
    with col_dl:
        st.markdown("<br>", unsafe_allow_html=True)
        st.download_button("📥 הורד תבנית CSV", data=csv_template,
                           file_name='bond_template.csv', mime='text/csv')
    with col_up:
    ...
    if uploaded_file:
        ...
        if err:
            ...
            _render_results(analyzer, final_score, nd_ebitda, curr_ratio, cov_ratio,
                            bond_name_single, ytm, duration, rating)
...
with tab_compare:
    ...
    with st.expander("➕ הוסף אג\"ח להשוואה", expanded=True):
        ...
        with c2:
            ...
            cmp_rating   = st.selectbox("דירוג",
                            ['AAA','AA','A','BBB','BB','B','CCC','CC','C','D','NR'],
                            key="cmp_rating", index=3)
        with c3:
...
with tab_manual:
    ...
    if st.button("🔎 חשב ציון", type="primary"):
        ...
        if errors:
            ...
            final_score = analyzer.calculate_final_score()
            _render_results(analyzer, final_score, nd_ebitda, curr_ratio, cov_ratio,
                            bond_name_manual, ytm, duration, rating)
```
...
def _render_results(analyzer, final_score, nd_ebitda, curr_ratio, cov_ratio,
bond_name, ytm, duration, rating):
st.divider()
...
m1.metric("חוב נטו / EBITDA",   round(nd_ebitda,  2),
          "מעל 4 = מסוכן" if nd_ebitda  > 4   else "✅ תקין", delta_color="inverse")
m2.metric("יחס שוטף (נזילות)", round(curr_ratio, 2),
          "מתחת ל-1 = סכנה" if curr_ratio < 1  else "✅ תקין")
m3.metric("יחס כיסוי ריבית",   round(cov_ratio,  2),
          "מתחת ל-1.5 = חלש" if cov_ratio < 1.5 else "✅ תקין")
...
# ── ייצוא ──
st.divider()
export_csv = build_export_csv(analyzer, final_score, nd_ebitda, curr_ratio, cov_ratio,
                              bond_name, ytm, duration, rating)
st.download_button(
