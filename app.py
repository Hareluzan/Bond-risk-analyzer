    ...
    with tab_compare:
        ...
        if not saved_records:
            ...
            with c_left:
                ...
                # --- מקרא צבעים מתוקן (המרת טקסט לריבועי רקע אמיתיים) ---
                palette = [GOLD, "#C0392B", "#2980B9", "#27AE60", "#8E44AD", "#E67E22", "#16A085"]
                ...
                for idx, record in enumerate(saved_records):
                    ...
                    # במקום להשתמש בטקסט שיכול להידרס על ידי חוקי CSS כלליים, בניתי בלוק קטן (div) שצבוע מבפנים
                    legend_html += f"""
                    <div style='display: flex; align-items: center; gap: 8px;'>
                        <div style='width: 14px; height: 14px; background-color: {color}; border-radius: 2px;'></div>
                        <span style='color: var(--cream) !important; font-size: 1rem; font-weight: bold;'>{bond_name_legend}</span>
                    </div>
                    """
                legend_html += "</div>"
